...
const withCmsLabel = (labelPrefix: string) => (pr: GitHubPull) => pr.labels.some(l => isCMSLabel(labelPrefix, l.name));
const withoutCmsLabel = (labelPrefix: string) => (pr: GitHubPull) => pr.labels.every(l => !isCMSLabel(labelPrefix, l.name));
...
export default class API {
    ...
    if (!pullRequest) {
      try {
        return {
          ...
          labels: [{ name: statusToLabel(this.labelPrefix, this.initialWorkflowStatus) }],
      ...
      pullRequest.labels = pullRequest.labels.filter(l => !isCMSLabel(this.labelPrefix, l.name));
      const cmsLabel =
        pullRequest.state === PullRequestState.Closed
          ? { name: statusToLabel(this.labelPrefix, this.initialWorkflowStatus) }
          : { name: statusToLabel(this.labelPrefix, 'pending_review') };
  ...
  async getBranchPullRequest(branch: string) {
    if (this.useOpenAuthoring) {
      ...
      const pullRequests = await this.getPullRequests(branch, PullRequestState.Open, withCmsLabel(this.labelPrefix));
  ...
  async retrieveUnpublishedEntryData(contentKey: string) {
    ...
    const label = pullRequest.labels.find(l => isCMSLabel(this.labelPrefix, l.name)) as { name: string };
    const status = labelToStatus(this.labelPrefix, label.name);
  ...
  async listUnpublishedBranches() {
    ...
    if (this.useOpenAuthoring) {
      ...
      const pullRequests = await this.getPullRequests(
        ...
        pr => !pr.head.repo.fork && withoutCmsLabel(this.labelPrefix)(pr),
      );
      let prCount = 0;
  ...
  async setPullRequestStatus(pullRequest: GitHubPull, newStatus: string) {
    const labels = [
      ...pullRequest.labels.filter(label => !isCMSLabel(this.labelPrefix, label.name)).map(l => l.name),
